...
% EEG_CHANNEL = 35;
% EEG_NOISE = 0.1;
for ii=1:1 %length(ACTIVE_CHANNEL)
    ...
    for kk=1:length(EEG_CHANNEL)
        for jj=1:length(EEG_NOISE)
            ...
            for TT=1:20
                ...
                Lhat = augmentL(y.L,nchan);
                % THIS REQUIRE UPDATE DUE TO CHANGING subid_eeg_Lpq
                % arguments changes
                [out_sys.ss.A, out_sys.ss.B, out_sys.ss.H, out_sys.ss.D, out_sys.ss.K, ...
                    out_sys.est.C, out_sys.est.C_L21_bic, out_sys.est.C_Lpq, ...
                    out_sys.est.C_L21, out_sys.est.bic_ind_Lpq, out_sys.est.bic_ind_L21,out_sys.est.kappa_ncvx,out_sys.est.kappa_cvx,out_sys.est.C_Lpqk,out_sys.est.C_L21k] =  ...
                    subid_eeg_Lpq_withkappa(y.data(:,end-Timepoints+1:end,TT),Lhat,[],2*ceil(nstate/y.r),nstate);
                

                [TP_1,TN_1,FP_1,FN_1] = ROC_eeg(out_sys.est.C,SYSTEM(TT).sys.sys_ss.C);
                [TP_2,TN_2,FP_2,FN_2] = ROC_eeg(out_sys.est.C_L21_bic,SYSTEM(TT).sys.sys_ss.C);
                [TP_3,TN_3,FP_3,FN_3] = ROC_eeg(out_sys.est.C_Lpqk,SYSTEM(TT).sys.sys_ss.C);
                [TP_4,TN_4,FP_4,FN_4] = ROC_eeg(out_sys.est.C_L21k,SYSTEM(TT).sys.sys_ss.C);
                ...
                TP_Lpqk = TP_Lpqk+TP_3;
                FP_Lpqk = FP_Lpqk+FP_3;
                TN_Lpqk = TN_Lpqk+TN_3;
                FN_Lpqk = FN_Lpqk+FN_3;
                TP_L21k = TP_L21k+TP_4;
                FP_L21k = FP_L21k+FP_4;
                TN_L21k = TN_L21k+TN_4;
                FN_L21k = FN_L21k+FN_4;                
            ...
            FPR_L21k = FP_L21k/(FP_L21k+TN_L21k);
            TPR_L21k = TP_L21k/(TP_L21k+FN_L21k);
            FPR_Lpqk = FP_Lpqk/(FP_Lpqk+TN_Lpqk);
            TPR_Lpqk = TP_Lpqk/(TP_Lpqk+FN_Lpqk);            
            ...
            sys_est.stat.cvx_kappa = [FPR_L21k TPR_L21k];
            sys_est.stat.ncvx_kappa = [FPR_Lpqk TPR_Lpqk];            
            
            sys_est.stat.cvx_ROC = [FPR_CVX TPR_CVX];
            sys_est.stat.ncvx_ROC = [FPR_nCVX TPR_nCVX];
            sys_est.r = y.r;
            sys_est.sigma_v = y.sigma_v;
            out_file = strcat(RESULT_SAVE_DIRECTORY,'\kappa_mhat',int2str(nchan),'_ss5_estimate_model_',int2str(ii),'_eeg_',int2str(EEG_CHANNEL(kk)),'_',EEG_NOISE{jj},'T',int2str(sys_est.T));
