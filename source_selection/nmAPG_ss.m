...
%           min_C (1/2)||V-LCW ||_F^2 + alpha * \sum  ||C_i^T||_2^q
...
% INPUT : L,W,V
%         alpha: regularization level
%         qq = only 1 (group-lasso) or 0.5 (non-convex group norm penalty)
%         STEP_SIZE : proximal stepsize, maximum is 1/[norm(L)^2*norm(W)^2]
%         IS_LINESEARCH = (0,1), if 1, Barzilai-Borwein linesearch rule is used
...
while k<IT_MAX
    ...
    if (((ERR<=(TOL*ERRprev))&& (k>3)) || (all(Ckp1==0,'all')))  %|| ((k>1) && (abs(objval-history.obj(k-1))<objTOL*objval))
        C = (Ckp1);
        FLAG = 0;
        fprintf('TOTAL_ITERATION: %d, objval:%10.4f\n \n',k,objval)
        break
    else
        C = (Ckp1);
        FLAG = -1;
