...
%               group_density : density of non-zero entries in source
%                               cluster level
%           
%	OUTPUTS
...
%                   m_inactive  :   number of inactive sources
%                   lag         :   VAR model's order
%                   nbutter     :   order of highpass/lowpass butterworth
%                                   filter (bandpass order will be 2xnbutter)
%                   density     :   density of non-zero entries in GC matrix
%                   roi_density :   density of non-zero entries in source
%                                   cluster level
%                   ind_active  :   indices of non-zero row in C
%                   n_source_cluster : number of source cluster (must less than m)
%                   ind_cluster_source : list of first index in each source cluster;
%                                        e.g. {[1,2,3],[4,5,6],[7]} represents 3 source cluster 
%                   sigma_w     :   covariance of the noise w(t)
%                   source_model0 : output state-space model
%                   F0          : GC matrix of source_model0
%                   indexGC     : non-zero index of F0
%                   flag        : 'fullGC' -> GC is estimated
%                               : 'BinaryGC' -> only GC pattern is returned
