...
% OUTPUT
% 'M.A'           : VAR coefficients with dimension (n x n x p)
% 'M.ind_nz'      : non-zero index of GC matrix
% 'M.block_size'  : size of each source cluster
% 'M.GC_ROI'      : GC pattern in source cluster level (only pattern)
% 'M.binary_F'    : GC matrix in binary
% 'M.ind_cluster_source' : list of first index in each source cluster;
% e.g. {[1,2,3],[4,5,6],[7]} represents 3 source cluster 
% ind_cluster_source in this case will be [1,4,7]
...
while MAX_EIG
    ...
    if opts.AUGMENT
        THRESH = 1;
        MAX_GAIN = 1;
        A(((abs(A)<THRESH)&(A~=0))&(off_ind3D))=MAX_GAIN.*sign(A(((abs(A)<THRESH)&(A~=0))&(off_ind3D)));
    end
