...
% 
% This experiment uses Source selection with L21 penalty (convex case)
% User should change line 32 or 35 (select the number of sources in
% consideration)
...
load([inpath,'data_ssvep_eeg.mat'])
load([inpath,'ssvep_leadfield.mat']) % originally Kindms.mat stores lead-field matrix, from Deirel
...
N = cell(ntrial,1);
for kk=1:ntrial
    [sys_est(kk).sys,sys_est(kk).C_out] = subid_eeg_L21(ssvep_eeg{kk}(select_chan,:),Lhat,ii,nhat);%    estimate 'ntrial' models
end

save([outpath,'ssvep_eeg_estimated_model_convex.mat'],'sys_est');


%% Compute GC indices & plotting

load([outpath,'ssvep_eeg_estimated_model_convex.mat']);


for kk=1:ntrial
bic_ind =  sys_est(kk).C_out.ind_chosen_L21.bic; %bic
aicc_ind =  sys_est(kk).C_out.ind_chosen_L21.aicc; %aicc

    A{kk} = sys_est(kk).sys.A;
    aicc.Chat{kk} = sys_est(kk).C_out.C_L21_CLS(:,:,aicc_ind);
    E = sys_est(kk).C_out.E_L21_CLS(:,:,aicc_ind);
    aicc.W{kk} = sys_est(kk).C_out.W_L21_CLS(:,:,aicc_ind);
    [aicc.N{kk},~,~] = noisecovest(E,Lhat,'homo');

    bic.Chat{kk} = sys_est(kk).C_out.C_L21_CLS(:,:,bic_ind);
    E = sys_est(kk).C_out.E_L21_CLS(:,:,bic_ind);
    bic.W{kk} = sys_est(kk).C_out.W_L21_CLS(:,:,bic_ind);
...
for kk=1:ntrial
RR.C_L21{kk,1} = sys_est(kk).C_out.C_L21;
RR.C_L21_CLS{kk,1} = sys_est(kk).C_out.C_L21_CLS;
RR.aicc_ind{kk,1} = sys_est(kk).C_out.ind_chosen_L21.aicc;
RR.bic_ind{kk,1} = sys_est(kk).C_out.ind_chosen_L21.bic;
end
...
save([outpath,'ssvep_rawresult_L21.mat'],'RR');
%========================================== END COMPUTATION ======================================================
